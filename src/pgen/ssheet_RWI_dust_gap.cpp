...
namespace {
Real sound_speed, gamma1, inv_gm1, d0, p0, amp, x1size, x2size, x3size, qshear, Omega0,
  ...
  for (int k=kl; k<=ku; k++) {
    for (int j=jl; j<=ju; j++) {
      for (int i=il; i<=iu; i++) {
        ...
        //Real pi = 3.1415926;
        //Real delta_gas_vel1 = 1e-5*(std::sin(pi*x2/0.005)-std::sin(2*pi*x2/0.005)-std::sin(3*pi*x2/0.005)-std::sin(4*pi*x2/0.005)-std::sin(5*pi*x2/0.005))*(std::cos(pi*x1/0.005)-std::cos(2*pi*x1/0.005)-std::cos(3*pi*x1/0.005)-std::cos(4*pi*x1/0.005)-std::cos(5*pi*x1/0.005));

        //Real delta_gas_vel1 = amp*sound_speed*(ran2(&iseed) - 0.5);
        //Real delta_gas_vel2 = 1e-5*(std::sin(pi*x2/0.005)-std::sin(2*pi*x2/0.005)-std::sin(3*pi*x2/0.005)-std::sin(4*pi*x2/0.005)-std::sin(5*pi*x2/0.005))*(std::cos(pi*x1/0.005)-std::cos(2*pi*x1/0.005)-std::cos(3*pi*x1/0.005)-std::cos(4*pi*x1/0.005)-std::cos(5*pi*x1/0.005)); //amp*sound_speed*(ran2(&iseed) - 0.5);
        //Real delta_gas_vel3 = 1e-5*(std::sin(pi*x2/0.005)-std::sin(2*pi*x2/0.005)-std::sin(3*pi*x2/0.005)-std::sin(4*pi*x2/0.005)-std::sin(5*pi*x2/0.005))*(std::cos(pi*x1/0.005)-std::cos(2*pi*x1/0.005)-std::cos(3*pi*x1/0.005)-std::cos(4*pi*x1/0.005)-std::cos(5*pi*x1/0.005)); //amp*sound_speed*(ran2(&iseed) - 0.5);

        Real delta_gas_vel1 = 0;
        Real delta_gas_vel2 = 0;
        Real delta_gas_vel3 = 0;
...
#pragma omp simd
      for (int i=il; i<=iu; ++i) {
        ...
        Real &gas_dens = pmb->phydro->u(IDN, kl - k, j, i);
        Real &gas_mom1 = pmb->phydro->u(IM1, kl - k, j, i);
        Real &gas_mom2 = pmb->phydro->u(IM2, kl - k, j, i);
        Real &gas_mom3 = pmb->phydro->u(IM3, kl - k, j, i);
        ...
        //gas_dens = dens_bump;
        gas_dens = dens_bump;//*std::exp(-SQR(x3)/(2.0*SQR(Hg)));
        gas_mom1 = gas_dens*0.0;
        gas_mom2 = gas_dens*gas_vel_bump - no_orb_adv*gas_dens*K_vel;
        gas_mom3 = gas_dens*0.0;

        if (NON_BAROTROPIC_EOS) {
          Real &gas_erg = pmb->phydro->u(IEN, k, j, i);
          gas_erg = SQR(sound_speed)*gas_dens*inv_gm1 + 0.5*(SQR(gas_mom1) + SQR(gas_mom2) + SQR(gas_mom3))/gas_dens;
        }
			}
...
#pragma omp simd
					for (int i=il; i<=iu; ++i) {
						...
						Real dens_bump = d0*(1.0 - A_bump*std::exp(-SQR(x1)/(2.0*SQR(sigma_bump))));
						Real gas_dens  = dens_bump;//*std::exp(-SQR(x3)/(2.0*SQR(Hg)));

            Real &dust_dens = pmb->pdustfluids->df_cons(rho_id, kl - k, j, i);
            Real &dust_mom1 = pmb->pdustfluids->df_cons(v1_id,  kl - k, j, i);
            Real &dust_mom2 = pmb->pdustfluids->df_cons(v2_id,  kl - k, j, i);
            Real &dust_mom3 = pmb->pdustfluids->df_cons(v3_id,  kl - k, j, i);
...
#pragma omp simd
      for (int i=il; i<=iu; ++i) {
        ...
        Real &gas_dens = pmb->phydro->u(IDN, ku + k, j, i);
        Real &gas_mom1 = pmb->phydro->u(IM1, ku + k, j, i);
        Real &gas_mom2 = pmb->phydro->u(IM2, ku + k, j, i);
        Real &gas_mom3 = pmb->phydro->u(IM3, ku + k, j, i);
...
#pragma omp simd
            ...
            Real &dust_dens = pmb->pdustfluids->df_cons(rho_id, ku + k, j, i);
            Real &dust_mom1 = pmb->pdustfluids->df_cons(v1_id,  ku + k, j, i);
            Real &dust_mom2 = pmb->pdustfluids->df_cons(v2_id,  ku + k, j, i);
            Real &dust_mom3 = pmb->pdustfluids->df_cons(v3_id,  ku + k, j, i);
