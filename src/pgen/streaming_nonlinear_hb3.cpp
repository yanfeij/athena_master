...
namespace {
Real amp, nwx, nwy, nwz, rhog0, p0; // amplitude, Wavenumbers
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  nwx    = pin->GetOrAddInteger("problem", "nwx",    1.0);
  nwy    = pin->GetOrAddInteger("problem", "nwy",    1.0);
  nwz    = pin->GetOrAddInteger("problem", "nwz",    1.0);
  ipert  = pin->GetOrAddInteger("problem", "ipert",  1.0);
  ifield = pin->GetOrAddInteger("problem", "ifield", 1.0);
  ...
  EnrollUserHistoryOutput(7, HistoryBxBy, "<-BxBy>");
        ...
        for (int i=is; i<=ie; i++) {
          ...
          if (MAGNETIC_FIELDS_ENABLED) {
            if (ifield == 1) {
              ...
              pfield->b.x2f(ks,j,i) = B0*(std::sin(static_cast<Real>(kx)*x_dis));
              pfield->b.x3f(ks,j,i) = 0.0;
              if (i==ie) pfield->b.x1f(ks,j,ie+1) = 0.0;
              if (j==je) pfield->b.x2f(ks,je+1,i) = B0*(std::sin(static_cast<Real>(kx)*x_dis));
            } else if (ifield == 2) {
              pfield->b.x1f(ks,j,i) = 0.0;
...
Real DustVel1Maximum(MeshBlock *pmb, int iout) {
  ...
  for (int k=ks; k<=ke; k++) {
    for (int j=js; j<=je; j++) {
      for (int i=is; i<=ie; i++) {
        Real &dust_vel1 = df_prim(IM1, k, j, i);
...
Real DustVel2Maximum(MeshBlock *pmb, int iout) {
  ...
  for (int k=ks; k<=ke; k++) {
    for (int j=js; j<=je; j++) {
      for (int i=is; i<=ie; i++) {
        Real &dust_vel2 = df_prim(IM2, k, j, i);
...
Real DustVel3Maximum(MeshBlock *pmb, int iout) {
  ...
  for (int k=ks; k<=ke; k++) {
    for (int j=js; j<=je; j++) {
      for (int i=is; i<=ie; i++) {
        Real &dust_vel3 = df_prim(IM3, k, j, i);
