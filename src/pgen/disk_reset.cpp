    ...
    for (int j=js; j<=je; ++j) {
      ...
      for (int i=is; i<=ie; ++i) {
        ...
        if (NON_BAROTROPIC_EOS) {
          ...
          phydro->u(IEN,k,j,i) = p_over_r*(1.+0.1*std::sin(phi)) * phydro->u(IDN,k,j,i)/(gamma_gas - 1.0);
  ...
  for (int k = pmb->ks; k <= pmb->ke; ++k) {
    for (int j = pmb->js; j <= pmb->je; ++j) {
      for (int i = pmb->is; i <= pmb->ie; ++i) {
        ...
        // reset density and momenta to the initial conditions
        // Real ekin = 0.5*(SQR(prim(IM1,k,j,i))+SQR(prim(IM2,k,j,i))
        //      +SQR(prim(IM3,k,j,i)))*prim(IDN,k,j,i);
        // Real eint = prim(IEN,k,j,i)/(gamma_gas-1.0);
        Real rad, phi, z;
        GetCylCoord(pmb->pcoord, rad, phi, z, i, j, k);
        cons(IDN,k,j,i) = DenProfileCyl(rad, phi, z);
        Real vel = VelProfileCyl(rad, phi, z);
        if (pmb->porb->orbital_advection_defined)
          vel -= pmb->porb->OrbitalVelocity(pmb->porb, pmb->pcoord->x1v(i), pmb->pcoord->x2v(j), pmb->pcoord->x3v(k));
        cons(IM1,k,j,i) = 0.0;
        if (std::strcmp(COORDINATE_SYSTEM, "cylindrical") == 0) {
          cons(IM2,k,j,i) = cons(IDN,k,j,i)*vel;
          cons(IM3,k,j,i) = 0.0;
        } else if (std::strcmp(COORDINATE_SYSTEM, "spherical_polar") == 0) {
          cons(IM2,k,j,i) = 0.0;
          cons(IM3,k,j,i) = cons(IDN,k,j,i)*vel;
        }
        //eint = T_gas*cons(IDN,k,j,i)/(gamma_gas-1.0); // reset internal energy
        ekin = 0.5*(SQR(cons(IM1,k,j,i))+SQR(cons(IM2,k,j,i))
              +SQR(cons(IM3,k,j,i)))/cons(IDN,k,j,i);
