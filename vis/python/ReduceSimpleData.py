...
# get rho, v1, v2, v3, B1, B2, B3, kappa_s, kappa_a
  ...
  quantities.append('Ek1')
  quantity_datasets.append('None')
  quantity_indices.append(0)

  quantities.append('Ek2')
  quantity_datasets.append('None')
  quantity_indices.append(0)

  quantities.append('Ek3')
  quantity_datasets.append('None')
  quantity_indices.append(0)
  ...
  quantities.append('gast')
  quantity_datasets.append('None')
  quantity_indices.append(0)


  quantities.append('Sr1')
  quantity_datasets.append('None')
  quantity_indices.append(0)

  quantities.append('Sr2')
  quantity_datasets.append('None')
  quantity_indices.append(0)

  quantities.append('St1')
  quantity_datasets.append('None')
  quantity_indices.append(0)

  quantities.append('St2')
  quantity_datasets.append('None')
  quantity_indices.append(0)
...
# Calculate fine-level offsets
    ...
    vt_data=f[spec_datasets[3]][spec_indices[3],block_num,:]
    vp_data=f[spec_datasets[4]][spec_indices[4],block_num,:]
    B1_data=f[spec_datasets[12]][spec_indices[12],block_num,:]
    B2_data=f[spec_datasets[13]][spec_indices[13],block_num,:]
    B3_data=f[spec_datasets[14]][spec_indices[14],block_num,:]
    ...
    bdotv = vr_data*B1_data+vt_data*B2_data+vp_data*B3_data
    pbsquar = 2.0 * PB_data



    BrBphi = np.multiply(B1_data,B3_data)   
    BtBphi = np.multiply(B2_data,B3_data)   

    ek1 = np.multiply(vr_data,vr_data)  
    ek1 = 0.5 * np.multiply(rho_data, ek1)

    ek2 = np.multiply(vt_data,vt_data)  
    ek2 = 0.5 * np.multiply(rho_data, ek2)

    ek3 = np.multiply(vp_data,vp_data)  
    ek3 = 0.5 * np.multiply(rho_data, ek3)

    vrvp = np.multiply(vr_data,vp_data)  
    vtvp = np.multiply(vt_data,vp_data)  

    rhovrvp = np.multiply(rho_data, vrvp)
    rhovtvp = np.multiply(rho_data, vtvp)

    rhovp = np.multiply(rho_data, vp_data)    
  ...
  # Assign values
    for q,dataset,index in zip(quantities,quantity_datasets,quantity_indices):
      if q == 'Sr1':
         oridata=vr_data*pbsquar
      elif q == 'Sr2':
         oridata=-bdotv*B1_data
      elif q == 'St1':
         oridata=vt_data*pbsquar
      elif q == 'St2':
         oridata=-bdotv*B2_data
      elif q == 'PB1':
         oridata = PB1_data
      elif q == 'PB2':
         oridata = PB2_data
         ...
         oridata=pgas_data/rho_data
      elif q=='rhovthetavphi':
         oridata=rhovtvp 
      elif q=='rhovphi': 
         oridata=rhovp
      elif q=='rhovtheta': 
         oridata=np.multiply(rho_data,vt_data)
      else:
